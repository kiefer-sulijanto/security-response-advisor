...
def main():
    ...
    parser.add_argument(
        "--realtime",
        action="store_true",
        help="Sleep between sent frames to simulate real camera timing",
    )
    ...
    while True:
        ...
        if frame_index % interval == 0:
            ...
            timestamp = (base_time + timedelta(seconds=sent_count * args.every_seconds)).isoformat(timespec="seconds")
            ...
            try:
                ...
                for result in data.get("results", []):
                    ...
                    advisory_timing = advisory.get("_timing", {})
                    if advisory_timing.get("openai_advisory_seconds") is not None:
                        openai_times.append(advisory_timing["openai_advisory_seconds"])
                ...
                print(
                    ...
                    f"route_total={timing.get('total_seconds')} | "
                    f"decode={timing.get('decode_seconds')} | "
                    f"pipeline={timing.get('pipeline_seconds')} | "
                    f"persist={timing.get('incident_persist_seconds')} | "
                    f"yolo={debug.get('yolo_seconds')} | "
                    f"fight_total={debug.get('fight_detection_seconds')} | "
                    f"remote_req={debug.get('remote_fight_request_seconds')} | "
                    f"remote_total={debug.get('remote_fight_total_seconds')} | "
                    f"openai={openai_times} | "
                    f"incidents={data.get('incidents_created')} | "
                    f"person_count={debug.get('person_count')} | "
                    f"fight_buffer={debug.get('fight_buffer_size')} | "
                    f"fight_frames={debug.get('fight_frames_sent')} | "
                )
